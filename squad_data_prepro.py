...
for i in version:
    if i == 1 :
        train_dir=setting.train_v1_filename
        dev_dir  =setting.dev_v1_filename
        SQUAD_dir=setting.SQUAD_v1_dir
        SQUAD_train_dir = setting.Train_v1_dir
        SQUAD_dev_dir = setting.DEV_v1_dir
        print('Preprocess SQUAD v1.1 Data')
    elif i == 2:
        train_dir=setting.train_v2_filename
        dev_dir  =setting.dev_v2_filename
        SQUAD_dir=setting.SQUAD_v2_dir
        SQUAD_train_dir = setting.Train_v2_dir
        SQUAD_dev_dir = setting.DEV_v2_dir
        print('Preprocess SQUAD v2.0 Data')
    train=load_squad_file(os.path.join(SQUAD_dir,train_dir))
    dev =load_squad_file( os.path.join(SQUAD_dir,dev_dir))
    try:
      nlp=spacy.load('en')
    except:
    ...
    char_Vocab_simple.save(os.path.join(SQUAD_dir,setting.char_simple_vocab_w2i_file),os.path.join(SQUAD_dir,setting.char_simple_vocab_i2w_file))
    ...
    char_Vocab_all.save(os.path.join(SQUAD_dir,setting.char_all_vocab_w2i_file) ,os.path.join(SQUAD_dir,setting.char_all_vocab_i2w_file))
    ...
    word_Vocab.save(os.path.join(SQUAD_dir,setting.word_vocab_w2i_file),os.path.join(SQUAD_dir,setting.word_vocab_i2w_file))
    ...
    np.save(os.path.join(SQUAD_train_dir,setting.train_P_file)            ,train_P)
    np.save(os.path.join(SQUAD_train_dir,setting.train_P_pos_file)        ,train_P_pos)
    np.save(os.path.join(SQUAD_train_dir,setting.train_P_ner_file)        ,train_P_ner)
    np.save(os.path.join(SQUAD_train_dir,setting.train_P_tag_file)        ,train_P_tag)

    np.save(os.path.join(SQUAD_train_dir,setting.train_Q_file)            ,train_Q)
    np.save(os.path.join(SQUAD_train_dir,setting.train_Q_pos_file)        ,train_Q_pos)
    np.save(os.path.join(SQUAD_train_dir,setting.train_Q_ner_file)        ,train_Q_ner)
    np.save(os.path.join(SQUAD_train_dir,setting.train_Q_tag_file)        ,train_Q_tag)

    np.save(os.path.join(SQUAD_train_dir,setting.train_P_char_simple_file),train_P_char_simple)
    np.save(os.path.join(SQUAD_train_dir,setting.train_Q_char_simple_file),train_Q_char_simple)
    np.save(os.path.join(SQUAD_train_dir,setting.train_P_char_all_file)  ,train_P_char_all)
    np.save(os.path.join(SQUAD_train_dir,setting.train_Q_char_all_file)  ,train_Q_char_all)

    np.save(os.path.join(SQUAD_train_dir,setting.train_A_file)            ,train_A)

    np.save(os.path.join(SQUAD_train_dir,setting.train_Q_id_file)         ,q_id)

    with open(os.path.join(SQUAD_train_dir,setting.train_Q_id_to_qid_file), 'wb') as f:
        pickle.dump(id_to_qid, f)

    np.save(os.path.join(SQUAD_dev_dir,setting.dev_P_file)            ,dev_P)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_P_pos_file)        ,dev_P_pos)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_P_ner_file)        ,dev_P_ner)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_P_tag_file)        ,dev_P_tag)

    np.save(os.path.join(SQUAD_dev_dir,setting.dev_Q_file)            ,dev_Q)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_Q_pos_file)        ,dev_Q_pos)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_Q_ner_file)        ,dev_Q_ner)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_Q_tag_file)        ,dev_Q_tag)

    np.save(os.path.join(SQUAD_dev_dir,setting.dev_P_char_simple_file),dev_P_char_simple)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_Q_char_simple_file),dev_Q_char_simple)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_P_char_all_file)  ,dev_P_char_all)
    np.save(os.path.join(SQUAD_dev_dir,setting.dev_Q_char_all_file)  ,dev_Q_char_all)

    np.save(os.path.join(SQUAD_dev_dir,setting.dev_A_file)            ,dev_A)

    np.save(os.path.join(SQUAD_dev_dir,setting.dev_Q_id_file)         ,dev_q_id)

    with open(os.path.join(SQUAD_dev_dir,setting.dev_Q_id_to_qid_file), 'wb') as f:
    ...
    np.save(os.path.join(SQUAD_dir,setting.SQUAD_Word_Embedding_file),squad_word_embedding)
    ...
    np.save(os.path.join(SQUAD_dir,setting.SQUAD_Char_simple_Embedding_file),squad_char_simple_embedding)
    np.save(os.path.join(SQUAD_dir,setting.SQUAD_Char_all_Embedding_file)   ,squad_char_all_embedding)
